...
import postsUrlNames from "../../../public/posts/js/postsUrlNames.json";
import HireForm from "../../../components/HireForm";
import Modal from "../../../components/Modal";

const jsPost = ({ metaData }) => {
  const [modalShow, setModalShow] = useState(false);
  const router = useRouter();
  const postId = router.query.postId;
  const PostPage = dynamic(() => import(`../../../public/posts/js/PostPage`));
  const imageUrl = `/posts/js/${postId}/header.png`;
  ...
  `,
        ...
        <link
          rel="canonical"
          href={`https://bafreelancing.com/Journeys/js/${postId}`}
        />
...
export async function getStaticProps(ctx) {
  const metaPath = path.join(
    process.cwd(),
    `public/posts/js/${ctx.params.postId}/meta.json`
  );
